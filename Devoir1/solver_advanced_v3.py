...
def reduce_last(final_walls, init_walls):
    ...
    chosen_walls = [(i, w) for i, w in enumerate(init_walls)]
    # sort by largest piece
    chosen_walls.sort(key=lambda x: x[1].pieces[-1]['width'] * x[1].pieces[-1]['height'], reverse=True)
    chosen_walls = chosen_walls[:min(10, len(init_walls))]

    artpieces = [(i, w.pieces[-1]) for i, w in chosen_walls]
    available = []
    ...
    if available:
        selected = parallel_select(final_walls, available)
...
def parallel_get_available(final_walls, idx, init_idx, artpiece):
    res = (
        ...
        init_idx,
        artpiece['id'],
        *final_walls[idx].get_available(artpiece['width'], artpiece['height']),
        artpiece['width'],
        artpiece['height']
    )
    return res
